...
ruta <-  "C:\\Users\\Encarni\\Desktop\\TFM\\Prediccion\\autoridades\\lasPalmas.xlsx"
datos_laspalmas <- read_excel(ruta)
names(datos_laspalmas)
datos_laspalmas
total_ton = ts(datos_laspalmas$Total_toneladas, freq=12, start = c(2022,1), end = c(2024,4))
str(total_ton)
total_ton


plot(total_ton, main='Tráfico de Toneladas mensuales en Las Palmas', xlab='Año', ylab = 'Toneladas', col="steelblue1")
...
tseries::adf.test(total_ton) ## p-value: 0.3903
adf.test(total_ton) 
...
## Con diferencia
dnot<-diff(total_ton)
plot(dnot, type="l", main="Tráfico en Toneladas mensuales en Las Palmas", sub=NA, xlab="Meses/Año", ylab="Toneladas", col="steelblue1")
...
tseries::adf.test(dnot) #p-valor=0,01 

## Se hace una segunda diferenciación
diff_2<-diff((total_ton), differences = 2)
plot(x=diff_2,  type="l", main="Tráfico en Toneladas mensuales en Las Palmas", sub=NA, xlab="Meses/Año", ylab="Toneladas", col="steelblue1")
acf(diff_2, main="Función de Autocorrelación Simple", col="steelblue1",xlab =,ylab=) #q=2
pacf(diff_2, main='Función de Autocorrelación Parcial', col="steelblue1",xlab =,ylab= ) #p=0   2
...
## Estimación Modelo ARIMMA (0,2,2) buena Las Palmas
astsa::sarima(train, 0,2,2) #  2 diferenciación aic=29.18
astsa::sarima(train, 2,2,2)  #aic=29.34
astsa::sarima(train, 1,1,1) # Mejor Las palmas AIC= 28,7436
 


ajuste2<-astsa::sarima(total_ton, 0,1,0)
Box.test(residuals(ajuste2$fit),type="Ljung-Box")

# Verificación
model1<-arima(x=train, order = c(0,2,2))# Las Palmas mEJOR. NOS QUEDAMOS 0,2,2
summary(model1) # MAE: 271184.7


model2<-arima(x=train, order= c(1,1,2))
summary((model2)) # MAE: 281887.5

model3 <- arima(x=train, order = c(2,2,2))
summary(model3) #MAE=270458.7


## Predicción

forecast::forecast(model1, h=12)
plot(forecast::forecast(model1, h=12), main = "Predicción Las Palmas")

forecast::forecast(model3,h=12)

plot(forecast::forecast(model3,h=12), main="Predicción Las Palmas")
