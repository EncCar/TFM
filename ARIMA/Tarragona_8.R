...
ruta <-  "C:\\Users\\Encarni\\Desktop\\TFM\\Prediccion\\autoridades\\tarragona.xlsx"
datos_tarragona <- read_excel(ruta)
names(datos_tarragona)
datos_tarragona
total_ton = ts(datos_tarragona$Total_toneladas, freq=12, start = c(2022,1), end = c(2024,4))
str(total_ton)
total_ton


plot(total_ton, main='Tráfico de Toneladas mensuales en Tarragona', xlab='Año', ylab = 'Toneladas', col="steelblue1")
...
tseries::adf.test(total_ton) ## p-value: 0.0609
...
## Con diferencia
dnot<-diff(total_ton)
plot(dnot, type="l", main="Tráfico en Toneladas mensuales en Tarragona", sub=NA, xlab="Meses/Año", ylab="Toneladas", col="steelblue1")
acf(dnot, main="Función de Autocorrelación Simple", col="steelblue1") #q=2  3
pacf(dnot, main="Función de Autocorrelación Parcial", col="steelblue1")# P= 1  2

plot(train, ylab="Toneladas", main="Time")

tseries::adf.test(dnot) #p-valor= 0.0299
...
pacf(diff_2, main='Función de Autocorrelación Parcial', col="steelblue1",xlab =,ylab= ) #p=2  0
...
## Estimación Modelo ARIMMA (2,1,1) Tarragona
astsa::sarima(train, 2,1,1) #  AIC = 28,54
astsa::sarima(train, 2,1,3) # AIC= 28.95
astsa::sarima(train, 0,2,2) # AIC= 29,10069
...
# Verificación
model1<-arima(x=train, order = c(2,1,1))# 
summary(model1) # MAE: 334344.7

model2<-arima(x=train, order= c(2,1,3))
summary(model2) # MAE: 246579.8

model3<-arima(x=train, order= c(0,2,2))
summary(model3) # MAE: 309495.2

model4<-arima(x=train, order= c(2,1,2))
summary(model4) # MAE: 2298838.7

## Predicción
forecast::forecast(model2,h=12)

plot(forecast::forecast(model2,h=12), main="Predicción Tarragona")
