...
ruta <-  "C:\\Users\\Encarni\\Desktop\\TFM\\Prediccion\\autoridades\\bilbao.xlsx"
datos_bilbao <- read_excel(ruta)
names(datos_bilbao)
datos_bilbao
total_ton = ts(datos_bilbao$Total_toneladas, freq=12, start = c(2022,1), end = c(2024,4))
str(total_ton)
total_ton


plot(total_ton, main='Tráfico de Toneladas mensuales en Bilbao', xlab='Año', ylab = 'Toneladas', col="steelblue1")
...
## Con diferencia
dnot<-diff(total_ton)
plot(dnot, type="l", main="Tráfico en Toneladas mensuales en Bilbao", sub=NA, xlab="Meses/Año", ylab="Toneladas", col="steelblue1")
acf(dnot, main="Función de Autocorrelación Simple", col="steelblue1") #q=2
pacf(dnot, main="Función de Autocorrelación Parcial", col="steelblue1")# P=1  3

plot(train, ylab="Toneladas", main="Time")

tseries::adf.test(dnot) #p-valor=0,2344
...
pacf(diff_2, main='Función de Autocorrelación Parcial', col="steelblue1",xlab =,ylab= ) #p=2  3
...
## Estimación Modelo ARIMMA (1,1,2) Bilbao
astsa::sarima(train, 0,2,2) #  AIC = 28,9037
astsa::sarima(train, 2,2,2) # AIC= 28.96266
astsa::sarima(train, 3,2,2)  # AIC = 29.06257
...
summary(model1) # MAE: 245766.6

model2<-arima(x=train, order= c(2,2,2))
summary(model2) # MAE: 215488.9

model3<-arima(x=train, order = c(3,2,2)) #MAE: 215817.1
summary(model3)
...
plot(forecast::forecast(model2,h=12), main="Predicción Bilbao")
